...
EM_PORT_API(int*)
getIntPtr() { return &g_int; }

EM_PORT_API(double*)
getDoublePtr() { return &g_double; }
...
EM_PORT_API(void)
printInt(int a) { printf("C{printInt() a:%d}\n", a); }

EM_PORT_API(void)
printFloat(float a) { printf("C{printFloat() a:%f}\n", a); }
...
EM_PORT_API(void)
freeBuf(void* buf) { free(buf); }

EM_PORT_API(void)
jsPrintFib(int* ptr, int count);
...
EM_PORT_API(double)
addDouble(double a, int b) { return a + (double)b; }

EM_PORT_API(int)
sumUint8(uint8_t* ptr, int count)
{
    int total = 0, temp;
    for (int i = 0; i < count; i++)
    {
